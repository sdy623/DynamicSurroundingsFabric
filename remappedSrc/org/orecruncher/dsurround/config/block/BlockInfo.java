...
public class BlockInfo {
    ...
    private static float estimateReflectance(BlockState state) {
        ...
        if (result == null) {
            var pathString = state.getRegistryEntry().getKey().map(k -> k.getValue().getPath()).orElse(null);
            if (pathString != null) {
    ...
    private static float estimateOcclusion(BlockState state) {
        ...
        if (result == null)
            result = state.isOpaque() ? Occlusion.DEFAULT : Occlusion.DEFAULT_TRANSLUCENT;
