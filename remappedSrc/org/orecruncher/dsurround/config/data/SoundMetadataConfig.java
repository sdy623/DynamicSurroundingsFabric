...
public record SoundMetadataConfig(Optional<String> title, Optional<String> subtitle, Optional<SoundCategory> category, List<CreditEntry> credits) {

    private static final Map<String, SoundCategory> SOUND_SOURCE_BY_NAME = Arrays.stream(SoundCategory.values()).collect(Collectors.toMap(SoundCategory::getName, (category) -> category));
    public static final Codec<SoundCategory> SOUND_SOURCE_CODEC = Codec.STRING.comapFlatMap(DataResult.partialGet(SOUND_SOURCE_BY_NAME::get, () -> "unknown sound category type"), SoundCategory::getName);
