...
public class BlockLibrary implements IBlockLibrary {
    ...
    public Stream<String> dumpBlockStates() {
        return RegistryUtils.getRegistry(RegistryKeys.BLOCK).orElseThrow()
                .stream()
    ...
    public Stream<String> dumpBlocks(boolean noStates) {
        var blockRegistry = RegistryUtils.getRegistry(RegistryKeys.BLOCK).orElseThrow();
        var entrySet = blockRegistry.getEntrySet();
        return entrySet.stream().map(kvp -> formatBlockOutput(kvp.getKey().getValue(), kvp.getValue(), noStates)).sorted();
    }
    ...
    private static String formatBlockTagOutput(TagKey<Block> blockTag, Set<Block> blocks) {
        ...
        builder.append("Tag: ").append(blockTag.id());
        blocks.stream()
                .map(b -> Objects.requireNonNull(blockRegistry.getId(b)).toString())
                .sorted()
    ...
    private String formatBlockOutput(Identifier id, Block block, boolean noStates) {
        var entry = RegistryUtils.getRegistryEntry(RegistryKeys.BLOCK, block).orElseThrow();
