...
public class ItemLibrary implements IItemLibrary {
    ...
    public Stream<String> dump() {
        var itemRegistry = RegistryUtils.getRegistry(RegistryKeys.ITEM).map(Registry::getEntrySet).orElseThrow();
        return itemRegistry.stream().map(kvp -> formatItemOutput(kvp.getKey().getValue(), kvp.getValue())).sorted();
    }
    ...
    private static @Nullable ISoundFactory resolveEquipableStepSound(ItemStack stack) {
        ...
        if (sound != null)
            return SoundFactoryBuilder
                    .create(sound)
                    .category(SoundCategory.PLAYERS).volume(0.07F).pitch(0.8F, 1F).build();
        return null;
    }
        ...
        if (itemClassType == ItemClassType.NONE) {
            ...
            if (itemEquipSound != null)
                return SoundFactoryBuilder
                        ...
                        .category(SoundCategory.PLAYERS).volume(0.5F).pitch(0.8F, 1.2F).build();
    ...
    private String formatItemOutput(Identifier id, Item item) {
        var tags = RegistryUtils.getRegistryEntry(RegistryKeys.ITEM, item)
                .map(e -> {
