...
public final class SoundLibrary implements ISoundLibrary {
    ...
    @Override
    public ISoundFactory getSoundFactoryForMusic(MusicSound music) {
        return this.soundFactories.computeIfAbsent(music.getSound().value().getId(), loc -> SoundFactoryBuilder.create(music).build());
    }
    ...
    @Override
    public float getVolumeScale(SoundCategory category, Identifier sound) {
        ...
        if (category == SoundCategory.AMBIENT && sound.getNamespace().equals(Library.MOD_ID))
    ...
    private void registerSoundFile(DiscoveredResource<Map<String, SoundMetadataConfig>> soundFile) {
        ...
        result.forEach((key, value) -> {
            ...
            final Identifier loc = new Identifier(soundFile.namespace(), key);
