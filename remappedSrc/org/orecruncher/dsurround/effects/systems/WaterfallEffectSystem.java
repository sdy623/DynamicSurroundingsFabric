...
public class WaterfallEffectSystem extends AbstractEffectSystem implements IEffectSystem {
    ...
    static {
        ...
        var factory = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/0")).orElseThrow();
        Arrays.fill(ACOUSTICS, factory);

        ACOUSTICS[2] = ACOUSTICS[3] = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/1")).orElseThrow();
        ACOUSTICS[4] = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/2")).orElseThrow();
        ACOUSTICS[5] = ACOUSTICS[6] = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/3")).orElseThrow();
        ACOUSTICS[7] = ACOUSTICS[8] = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/4")).orElseThrow();
        ACOUSTICS[9] = ACOUSTICS[10] = soundLibrary.getSoundFactory(new Identifier(Constants.MOD_ID, "waterfalls/5")).orElseThrow();
    }
    ...
    public void tick(Predicate<IBlockEffect> processingPredicate) {
        ...
        for (var system : this.systems.values()) {
            ...
            if (desiredLocations.contains(posIndex)) {
                if (sound == null) {
                    int idx = MathHelper.clamp(waterFallEffect.getStrength(), 0, ACOUSTICS.length - 1);
    ...
    protected Set<Long> getDesiredWaterfallSoundLocations() {
        ...
        if (this.systems.size() <= SOUND_INSTANCE_CAP)
            return this.systems.values().stream()
                    .filter(e -> shouldPlaySoundFilter(player.method_48926(), e))
    ...
    private static boolean shouldPlaySoundFilter(World world, IBlockEffect effect) {
    ...
    @NotNull
    private static WaterfallEffect createWaterfallEffect(World world, BlockState state, BlockPos pos) {
    ...
    private static boolean isValidWaterfallSource(World world, BlockPos pos) {
    ...
    private static boolean isBoundedLiquid(World provider, BlockPos pos) {
        ...
        for (final Vec3i cardinal_offset : CARDINAL_OFFSETS) {
            ...
            if (fluidState.contains(FlowableFluid.FALLING))
                return false;
            final int height = fluidState.getLevel();
