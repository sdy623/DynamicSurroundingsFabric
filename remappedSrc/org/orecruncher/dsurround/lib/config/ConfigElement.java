...
public abstract class ConfigElement<T> {
    ...
    public Text getTooltip(Style style) {
        ...
        return resourceText
                .map(txt -> Text.literal(txt).fillStyle(style))
                .orElse(Text.literal("MISSING: " + key).fillStyle(STYLE_MISSING));
    }
    ...
    public static class PropertyValue<T> extends ConfigElement<T> {
        ...
        public Text getDefaultValueTooltip() {
            return Text.translatable("dsurround.config.tooltip.defaultValue", this.defaultValue).fillStyle(STYLE_DEFAULT);
        }
    }

    public static class BooleanValue extends PropertyValue<Boolean> {

        private static final Text YES = Text.translatable("gui.yes").withColor(ColorPalette.GREEN.getRgb());
        private static final Text NO = Text.translatable("gui.no").withColor(ColorPalette.RED.getRgb());
        ...
        public Text getDefaultValueTooltip() {
            ...
            return Text.translatable("dsurround.config.tooltip.defaultValue", text).fillStyle(STYLE_DEFAULT);
        }
    ...
    public static class IntegerValue extends PropertyValue<Integer> implements IRangeTooltip {
        ...
        @Override
        public Text getRangeTooltip() {
            return Text.translatable("dsurround.config.tooltip.range", this.getMinValue(), this.getMaxValue()).fillStyle(STYLE_RANGE);
        }
