...
public class ClientTagLoader {
    ...
    private <T> TagData<T> loadTagData(TagKey<T> tagKey, Set<TagKey<?>> visited) {
        ...
        if (holderSet.isPresent()) {
            ...
            for (var holder : data) {
                ...
                key.ifPresent(tResourceKey -> completeIds.add(tResourceKey.getValue()));
            }
        } else {
            ...
            if (!entries.isEmpty()) {
                ...
                var lookup = new TagEntry.ValueGetter<Identifier>() {
                    @Override
                    public @NotNull Identifier direct(@NotNull Identifier id) {
                        return id;
                    }
                ...
                for (TagEntry tagEntry : entries) {
                    tagEntry.resolve(lookup, completeIds::add);
                }
            }
        }
    ...
    private <T> Optional<Iterable<RegistryEntry<T>>> shortcutLookup(TagKey<T> tagKey) {
        var namespace = tagKey.id().getNamespace();
        ...
        if (this.serverType.isModded() || "minecraft".equals(namespace)) {
            ...
            if (registry.isEmpty())
                return Optional.empty();
            var holderSet = registry.get().getEntryList(tagKey);
            if (holderSet.isPresent())
