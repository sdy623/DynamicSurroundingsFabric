...
public class SystemsScanner extends CuboidScanner {
    ...
    @Override
    public void blockScan(World world, BlockState state, BlockPos pos, IRandomizer rand) {
        this.processIfEnabled(false, system -> system.blockScan(world, state, pos));
    }

    @Override
    public void blockUnscan(World world, BlockState state, BlockPos pos, IRandomizer rand) {
        this.processIfEnabled(false, system -> system.blockUnscan(world, state, pos));
    }

    public void gatherDiagnostics(Collection<Text> output) {
        this.systems.forEach(system -> {
            var text = system.gatherDiagnostics();
            if (!system.isEnabled())
                text += " (disabled)";
            output.add(Text.literal(text));
        });
    }
}
