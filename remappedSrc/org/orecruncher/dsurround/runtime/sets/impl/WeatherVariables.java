...
public class WeatherVariables extends VariableSet<IWeatherVariables> implements IWeatherVariables {
    ...
    public void update(IVariableAccess variableAccess) {
        if (GameUtils.isInGame()) {
            ...
            final var world = player.method_48926();
            this.rainIntensity = world.getRainGradient(1F);
            this.thunderIntensity = world.getThunderGradient(1F);
            this.isRaining = world.isRaining();
            this.isThundering = world.isThundering();
            this.temperature = this.seasonalInformation.getTemperature(world, player.getBlockPos());
            this.isFrosty = this.seasonalInformation.isColdTemperature(world, player.getBlockPos());
            this.canWaterFreeze = this.seasonalInformation.isSnowTemperature(world, player.getBlockPos());
        } else {
